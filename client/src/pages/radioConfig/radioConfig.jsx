...
function RadioConfig({ radios, setRadios }) {
  ...
  return (
    <div className="radio-page">
      <div className="topbar">
        ...
        <div className="topbar-right">
          <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              ...
              onClick={() =>
                loadConfig((loaded) => {
                  ...
                  nextId.current =
                    Math.max(
                      0,
                      ...normalized.map((r) => Number(getRadioUid(r)) || 0),
                    ) + 1;
            ...
            <button className="btn" onClick={() => downloadConfig(radios)}>
              Download config
            </button>
            <div
              style={{
      ...
      <div
        className={`cards-wrap ${radios.length === 0 ? "cards-wrap--empty" : ""}`}
      >
          ...
          {radios.map((r, i) => (
            ...
            >
              <RadioCard
                radio={r}
                index={i}
                onConfigChange={(index, paramIdx, value) =>
                  handleConfigChange(index, paramIdx, value, setRadios)
                }
                onStructChange={(index, text) =>
                  handleStructChange(index, text, setRadios)
                }
                onStructParse={(index) => {
                  handleStructParse(index, setRadios);
                  setPanelRadioId(radios[index].id);
                }}
                onFieldChange={(index, fieldIdx, value) =>
                  handleFieldChange(index, fieldIdx, value, setRadios)
                }
                onRemove={(index) =>
                  setRadioPendingDelete({ index, radio: radios[index] })
                }
                onConfigTypeChange={(index, pIdx, value) =>
                  handleConfigTypeChange(index, pIdx, value, setRadios)
                }
                onConfigKeyChange={(index, pIdx, value) =>
                  handleConfigKeyChange(index, pIdx, value, setRadios)
                }
      ...
      {ctxMenu && (
        ...
        >
          <li
            onClick={() => {
              setPanelRadioId(ctxMenu.radioId);
              setCtxMenu(null);
            }}
          >
            👁 Properties
